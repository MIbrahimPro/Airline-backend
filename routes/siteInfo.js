...
router.get("/public/terms", async (req, res) => {
    const { terms } = (await SiteInfo.findOne().lean()) || {};
...
router.post("/", authenticate, authorizeAdmin, async (req, res) => {
    ...
    if (!("aboutUsLong" in data)) data.aboutUsLong = existing.aboutUsLong;
    if (!("privacyPolicy" in data)) data.privacyPolicy = existing.privacyPolicy;
    if (!("terms" in data)) data.terms = existing.terms;
    if (!("booking" in data)) data.booking = existing.booking;
